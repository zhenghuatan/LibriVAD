...
def split_data_into_sequences(features, labels, sequence_length):
    num_frames, num_features = features.shape  # Shape of features is [num_frames, num_features]
    sequences_features = []
    sequences_labels = []
    num_full_sequences = num_frames // sequence_length  # Number of full sequences
    remainder_frames = num_frames % sequence_length  # Frames remaining after full sequenc
    start = 0
    end = sequence_length
    for i in range(num_full_sequences):
        #print(i, start, end)
        feature_sequence = features[start:end]  # Shape: (sequence_length, num_features)
        label_sequence = labels[start:end]  # Shape: (sequence_length)
        sequences_features.append(feature_sequence)
        sequences_labels.append(label_sequence.ravel())
        start =  end
        end = end + sequence_length 
    sequences_features = np.array(sequences_features)
    sequences_labels = np.array(sequences_labels)

    if  remainder_frames > 0:
        last_sequence_features = features[num_full_sequences * sequence_length:]
        last_sequence_labels = labels[num_full_sequences * sequence_length:]
        pad_length = sequence_length - remainder_frames
        padded_features = np.pad(last_sequence_features, ((0, pad_length), (0, 0)), mode='constant')
        #check
        tmp = np.squeeze(np.concatenate(sequences_labels, axis=0 ))
        tmp = np.hstack((tmp, last_sequence_labels.ravel()))
        total = np.sum(np.abs(np.matrix(tmp).T - labels))
        if total != 0.0:
            raise ValueError('Label sequence mismatch')
           
    else:
        padded_features = np.zeros((sequence_length, num_features))
        last_sequence_labels = np.zeros((sequence_length, num_features))

    return sequences_features, sequences_labels,  np.array(padded_features), np.array(last_sequence_labels)
...
def auc_acc(tstscp, model, device, sequence_length, cutThres):
   score =[]
   Lab = []
   fp = open('frame_info.txt','w')
   for fe in tstscp:
      #try:
         ...
         X_sequences, y_sequences, t3, t3_seq = split_data_into_sequences(sgn, max_voters_pred, sequence_length)

         num_frames, num_features = sgn.shape 
         remainder_frames = num_frames % sequence_length  # Frames remaining after full sequenc

         if sgn.shape[0] == max_voters_pred.shape[0]:
            tmpscore = []
            tmpLab = []
            for idx, (x_seq, y_seq) in enumerate(zip(X_sequences, y_sequences)):
                x_seq =  torch.from_numpy(x_seq)
                sgn = x_seq.to(device)
                sgn = sgn.unsqueeze(0)
                logit = model(sgn.float())
                logit = logit.squeeze(0)
                
                logit = np.asmatrix(logit.cpu().detach().numpy())
                tmpscore.append(logit)
                tmpLab.append(y_seq.ravel())
            if remainder_frames > 0:  #consider the reminder frames
               x_seq =  torch.from_numpy(t3)
               sgn = x_seq.to(device)
               sgn = sgn.unsqueeze(0)
               logit = model(sgn.float())
               logit = logit.squeeze(0)
               logit = logit[:remainder_frames,:]
               logit = np.asmatrix(logit.cpu().detach().numpy())
               
               tmpscore.append(logit)
               tmpLab.append(t3_seq.ravel())
            tmpscore = np.squeeze(np.concatenate(tmpscore, axis=0 ))
            tmpLab = np.squeeze(np.concatenate(tmpLab, axis=0 ))
            tmpLab = np.squeeze(np.asarray(tmpLab))
            if max_voters_pred.shape[0] == tmpscore.shape[0] and max_voters_pred.shape[0] == tmpLab.shape[0]and np.sum(np.abs(max_voters_pred.ravel() - tmpLab)) == 0.0:
                  score.append(tmpscore)
                  Lab.append(np.asmatrix(tmpLab))
                  fp.write("%s %d\n" %(fe.split(",")[2].strip('\n'), len(tmpscore)))
      #except ValueError:
      #     pass
   score = np.squeeze(np.concatenate(score, axis=0 ))
   Lab = np.squeeze(np.concatenate(Lab, axis=1))
   Lab = np.squeeze(np.asarray(Lab))
   print(score.shape, Lab.shape)
   tot=0
   corr=0
   ACC1=0
   if cutThres == 'True':
   ...
   fp.close()
   if cutThres == 'True':
             return muc1, round(ACC1,4), score.shape[0], thresholds, fpr, tpr, score[:,1], Lab
   else:
             return muc1, round(ACC1,4), score.shape[0], cutThres, fpr, tpr, score[:,1], Lab
